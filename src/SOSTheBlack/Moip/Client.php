...
class Client {
    ...
    public function send($credentials, $xml, $url='https://desenvolvedor.moip.com.br/sandbox/ws/alpha/EnviarInstrucao/Unica', $method='POST') {
        ...
        return App::make('\Artesaos\Moip\Response', [['resposta' => $ret, 'erro' => $err]]);
    }
    ...
    function curlPost($credentials, $xml, $url, $error=null) {
        ...
        if (!$error) {
            ...
            if ($info['http_code'] == "200")
                return App::make('\Artesaos\Moip\Response', [['response' => true, 'error' => null, 'xml' => $ret]]);
            elseif ($info['http_code'] == "500")
                return App::make('\Artesaos\Moip\Response', [['response' => false, 'error' => 'Error processing XML', 'xml' => null]]);
            elseif ($info['http_code'] == "401")
                return App::make('\Artesaos\Moip\Response', [['response' => false, 'error' => 'Authentication failed', 'xml' => null]]);
            else
                return App::make('\Artesaos\Moip\Response', [['response' => false, 'error' => $err, 'xml' => null]]);
        } else {
            return App::make('\Artesaos\Moip\Response', [['response' => false, 'error' => $error, 'xml' => null]]);
        }
    }
